...
public class ExcelCreate {
   ...
   public  HSSFRow hdRow = null;
	...
	public void addHeader(List rowvalues, boolean isFilter) throws Exception {
		...
		dtRow.setHeight((short)60);
		hdRow.setHeight((short)380);
		if (isFilter == true) {
	...
	public void addHeaderByPosition(List rowvalues, boolean isFilter,int rowId) throws Exception {
		listlength = rowvalues.size();
		// 设置字体
		HSSFFont workFont = wb.createFont();
		workFont.setFontName("微软雅黑");
		...
		HSSFRow dtRow = sheet.createRow((rowId));
		dtRow.setHeight((short)380);
		hdRow.setHeight((short)380);
		if (isFilter == true) {
			for (int i = 0; i < title.length; i++) {
				HSSFCell cell1 = hdRow.createCell(i);
...
//			new SimpleDateFormat("yyyy-M-d").format(new Date())
		...
		s.createSheet1("第二张系统报名表");// 第二张工作表
