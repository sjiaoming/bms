...
<script type="text/javascript">
	...
	function search(offset){
		var url = "procurementCheckISearch.action?pager.offset="+offset;
	...
	function checkPro(){
		...
		procurementCheckManager.checkPCStatus(tags,'04,08',function (data){
			//alert(data);
			if(data == 'no'){
				alert("请选择正确的审核状态");
				return false;
				...
				diag.URL = "initProcurementWay.action?pids="+tags+"&initFlag=manager";
				diag.Height=258;
				diag.Width=508;
				diag.CancelEvent = function(){
					document.getElementById("resetflag").value = "add";
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'procurementIWayMgr',1)}">
				<td><button  onclick='search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
				<td colspan="8">
				<c:if test="${my:method(login.id,'procurementIWayMgr',2)}">
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="procurementCheckISearch.action" export="currentPageNumber=pageNumber" maxPageItems="50" >
