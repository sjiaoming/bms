...
<script type="text/javascript">
	...
	function search(offset){
		var url = "procurementCheckMSearch.action?pager.offset="+offset;
	...
	function checkPro(){
		...
		procurementCheckManager.checkPCStatus(tags,'06,03',function (data){
			//alert(data);
			if(data == 'no'){
				alert("请选择正确的审核状态");
				return false;
				...
				var diag = new top.Dialog();
				diag.Title = "审核采购方案";
				//diag.URL = "businessManagement/planTrackMg/procurementWay_m_modify.jsp?pids="+tags;
				diag.URL = "initProcurementWay.action?pids="+tags+"&initFlag=chairMan";
		...
		<table>
			<tr>
				...
				<td><select id="checkStatus" name="pc.checkStatus">
						...
						<option value="01" <c:if test="${pc.checkStatus == '01'}">selected="selected"</c:if> >主管末审核</option>	
						<option value="02" <c:if test="${pc.checkStatus == '02'}">selected="selected"</c:if> >主管末通过审核</option>	
						<option value="08" <c:if test="${pc.checkStatus == '08'}">selected="selected"</c:if> >主管己审核</option>		
						<option value="09" <c:if test="${pc.checkStatus == '09'}">selected="selected"</c:if> >内控末通过审核</option>	
						<option value="03" <c:if test="${pc.checkStatus == '03'}">selected="selected"</c:if> >内控己审核</option>	
						<option value="04" <c:if test="${pc.checkStatus == '04'}">selected="selected"</c:if> >经理末通过审核</option>		
						<option value="05" <c:if test="${pc.checkStatus == '05'}">selected="selected"</c:if> >经理己审核</option>	
						<option value="06" <c:if test="${pc.checkStatus == '06'}">selected="selected"</c:if> >董事长末通过审核</option>	
				...
				<c:if test="${my:method(login.id,'procurementMWayMgr',1)}">
				<td><button  onclick='search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
				<!-- showProgressBar(); -->
				<td colspan="8">
				<c:if test="${my:method(login.id,'procurementMWayMgr',2)}">
				<button type="button"  onclick='top.Dialog.confirm("您确定要重新选择？",function(){checkPro()});' ><span class="icon_page">批量审核采购方案</span></button>
				</c:if>
				</td>
