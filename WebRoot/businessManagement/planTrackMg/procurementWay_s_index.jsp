...
<script type="text/javascript">
	...
	function search(offset){
		var url = "procurementCheckSSearch.action?pager.offset="+offset;
	...
	function editPC(pcId){
		var diag = new top.Dialog();
		diag.Title = "修改采购方案";
		diag.URL = "getProcurementCheck.action?tags="+pcId+"&pageType=s_index";
		diag.Height=480;
		diag.Width=1320;
		diag.MessageTitle="修改采购方案";
		diag.Message="修改采购方案";
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
			var inputValue = diag.innerFrame.contentWindow.document.getElementById('cflag').value;
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'procurementSWayMgr',1)}">
				<td><button  onclick='search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
				</tr>
			<tr>
				<td colspan="11"><font color="red"><b><div id="selectCount" ></div></b></font></td>
			</tr>
				<!-- <td colspan="8">
				<c:if test="${my:method(login.id,'procurementSWayMgr',2)}">
...
<thead>
	...
	<c:if test="${!empty pm.list}">
        <c:forEach items="${pm.list }" var="pch" varStatus="status">  
			...
			<td >${pch.director.name }</td>
			<td >
			<c:if test="${my:method(login.id,'procurementSWayMgr',1)}">
				<span class="img_edit hand" title="修改" onclick="editPC(${pch.id})"></span>
			</c:if>
			</td>
		</tr>
		</c:forEach>
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="procurementCheckSSearch.action" export="currentPageNumber=pageNumber" maxPageItems="50" >
