...
<script type="text/javascript">
	...
	function backPlans(){
		...
		if(tags != ""){
			tagsplit = tags.split(",");
			//alert("tagsplit   "+tagsplit);
			//alert("tagsplit.length   "+tagsplit.length);
			for(var i=0 ;i<tagsplit.length;i++){
				var temp =  tagsplit[i].split("*");
				//alert("temp   "+temp);
				result[i] = temp[0];
			}
		}
		if(result==""){
			...
			this.form1.action = "modifyPlansByIdsBack.action?tags="+result;
			this.form1.submit();
			return;
	...
	function openWin3(){
		...
		diag.URL = "initProcurementWay.action?pids="+result+"&initFlag=director";
		diag.Height=445;
		diag.Width=508;
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
	...
	function openWin4(){
		...
		var diag = new top.Dialog();
		diag.Title = "添加方案";
		diag.URL = "businessManagement/planTrackMg/procurementDate_modify.jsp?pids="+result;
		diag.Height=258;
		diag.Width=508;
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
	...
	function openWin7(){
		...
		var tagsplit = result.split(",");
		var diag = new top.Dialog();
		diag.Title = "添加方案";
		if(tagsplit.length > 1 ){
			diag.URL = "toModifySeekSource.action?modifyFlag=procurementRemark&tags="+result;
	...
	function getSelectedCount(){
		...
		if(tags != ""){
			...
			document.getElementById("selectCount").innerHTML = "已选择："+result.length+" 条";
		}else{
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			...
			<tr>
				...
				-->
				<button type="button"  onclick='top.Dialog.confirm("您确定要退回这些计划吗？",function(){backPlans()});' ><span class="icon_page">计划退回</span></button>
				<c:if test="${my:method(login.id,'procurementWayMgr',2)}">
				<button type="button"  onclick='top.Dialog.confirm("您确定添加采购方案吗？",function(){openWin3()});' ><span class="icon_page">添加采购方案</span></button>
				</c:if>
				</td>
...
<thead>
	...
	<c:if test="${!empty pm.list}">
        <c:forEach items="${pm.list }" var="plan" varStatus="index">  
			<tr >
			<td><input  id="tg"  name="checkbox" type="checkbox" value="${plan.id }*${plan.reportCompId }" onclick='getSelectedCount()'/></td>
			<td>${index.count }</td>
