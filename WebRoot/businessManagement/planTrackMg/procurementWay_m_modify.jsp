...
<input type="hidden" name="pageType" value="charMan" />
<div class="box1" panelWidth="500">
	<table class="tableStyle" formMode="true">
		...
		<td>审核董事长:</td>
			<td>
				<select id="chairManId" name="pc.chairMan.id" class="default">
					<option value="0">请选择</option>
					<c:if test="${!empty cpersons}">
					 	<c:forEach items="${cpersons }" var="cperson">  
							<option value="${cperson.id}" <c:if test="${pc.chairMan.id==cperson.id}"> selected="selected"</c:if>>${cperson.name }</option>					 
						</c:forEach>
					</c:if>
				</select>
			</td>
