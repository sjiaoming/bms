...
function getArrivalValue(field){
	...
	//alert("begain sum_total_arrivalNum   "+sum_total_arrivalNum);
	//var temp = document.getElementById("tab1").getElementsByName("pid");
	var temp = $("input[name='pid']");
	...
	if(temp.length>0){
		for(var i =0;i<temp.length;i++){
			...
			if(temp[i].value == pid ){
					...
					if(i == 0){
						if(temp1[1].value!=null){
							sum_total_arrivalNum += Number(temp1[1].value);
						}
					}else{
						if(temp1[i*5+1].value!=null){
							sum_total_arrivalNum += Number(temp1[i*5+1].value);
						}
					}
					//alert(i);
			}
		}
		//alert("after sum_total_arrivalNum   "+sum_total_arrivalNum);
	}
	field.cells[3].childNodes[0].value = total_arrivalNums - sum_total_arrivalNum >= 0 ? total_arrivalNums - sum_total_arrivalNum : 0;
...
<form action="?" id="form1" name="form1" >
	...
	<table  id="tab2" class="tableStyle" >
		...
		<c:if test="${!empty pc.plan}">
			...
			<tr>
				<td>${p.id}</td>
				<td>${p.planNum}</td>
				<td><span class="text_slice" style="width:50px;" title="${p.itemsName}">${p.itemsName}</span></td>
				<td><span class="text_slice" style="width:80px;" title="${p.supplier.name}">${p.supplier.name}</span></td>
				<td>${p.contractNum}
	...
	<table id="tab0" class="tableStyle" >
		...
		<c:if test="${!empty al}">
			...
			<tr>
				<td>${l.plan.planNum }[${l.plan.id}]</td>
				<td><span class="text_slice" style="width:150px;" title="${l.plan.itemsName }">${l.plan.itemsName }</span></td>
				<td><span class="text_slice" style="width:150px;" title="${l.plan.supplier.name}">${l.plan.supplier.name}</span></td>
				<td>${l.arrivalNum }
	...
	<table id="tab1" class="tableStyle" >	
		...
		<tr onclick="getArrivalValue(this)" id="tr1">
			...
			<td style="width:70px"><input type="hidden" name="pid" value="${p.id}"></input>
			<c:if test="${!empty pc.plan}">
				   [${p.planNum} ]
			</c:if>
			</td>
			<td style="width:205px">
			<span class="text_slice" style="width:150px;" title="${p.itemsName }">
			[ ${p.itemsName}] [${p.id}]</span>
			</td>
			<td><span class="text_slice" style="width:150px;" title="${p.supplier.name}">${p.supplier.name}</span></td>
			<td style="width:150px"><input type="text" name="arrivalNum" class=" validate[required]" onclick="javascript:flag=true;"/></td>
			<td style="width:150px"><input  id="arrivalDate" name="arrivalDate"  value="" class="date"/></td>
			<td style="width:150px"><input type="text" name="acceptanceNum" class=" validate[required]" onclick="javascript:flag=false;"/></td>
			<td style="width:150px"><input  id="acceptanceDate" name="acceptanceDate"  value="" class="date"/></td>
