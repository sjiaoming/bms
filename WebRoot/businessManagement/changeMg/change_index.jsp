...
function selectValidationReturnTags(isAlert){
	...
	function changeSuppliers(){
		...
		var diag = new top.Dialog();
		diag.Title = "计划变更";
		diag.URL = "initChangeSuppliers.action?tags="+tags;
		diag.Height=400;
		diag.Width=680;
		diag.MessageTitle="变更供应商";
		diag.Message="变更供应商";
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
			var inputValue = diag.innerFrame.contentWindow.document.getElementById('cflag').value;
	...
	//变更窗口${plan.itemsName }
	function changeReportComp(){
		var tags = selectValidationReturnTags(false);
		var diag = new top.Dialog();
		diag.Title = "计划变更";
		diag.URL = "initChangeReportComp.action?tags="+tags;
		diag.Height=400;
		diag.Width=680;
		diag.MessageTitle="变更提报单位";
		diag.Message="变更提报单位";
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
			var inputValue = diag.innerFrame.contentWindow.document.getElementById('cflag').value;
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<td>供应商：</td>
				<td><input type="text" name="supplierName" value="${params.supplierName }"/></td>
				<td>要求到货时间 从:</td>
				<td>
			...
			<tr>
				<td colspan="7">
				<button type="button"  onclick='top.Dialog.confirm("您确定要修改供应商吗？",function(){changeSuppliers()});' ><span class="icon_remove">批量修改供应商</span></button>
				<button type="button"  onclick='top.Dialog.confirm("您确定要修改提报单位吗？",function(){changeReportComp()});' ><span class="icon_add">批量修改提报单位</span></button>
				</td>
...
<thead>
	<tr>
		...
		<a herf="#" onclick="CancelAll()" >不</a>
				...
				<th width="40">单位</th>
				<th width="60">计划数量</th>
				<th width="60">预算金额(元)</th>
				<th width="80">寻源单价</th>
				<th width="80">寻源数量</th>
				<th width="80">采购方案</th>
				<th width="80">寻源金额(元)</th>
				<th width="100">物资需求部门</th>
	...
	<c:if test="${!empty pm.list}">
		...
		<tr >
			...
			<td >
			<c:if test="${plan.pc.procurementWay == '01' }">公开招标</c:if>
			<c:if test="${plan.pc.procurementWay == '02' }">邀请招标</c:if>
			<c:if test="${plan.pc.procurementWay == '03' }">竞争性谈判</c:if>
			<c:if test="${plan.pc.procurementWay == '04' }">单一来源</c:if>
			<c:if test="${plan.pc.procurementWay == '05' }">寻比价</c:if>
			</td>
