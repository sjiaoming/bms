...
<script type="text/javascript">
	...
	function search(offset){
		var url = "searchOperatorProcurements.action?pager.offset="+offset;
	...
	function setProcurementOperator(){
		var tags = selectValidationReturnTags(true);
		if(tags==""){
			top.Dialog.alert("请选择采购方案");
			return;
		...
		var diag = new top.Dialog();
		diag.Title = "批量设置";
		diag.URL = "initSetProcurementOperator.action?tags="+tags+"&businessGroupId="+bgId;
		diag.Height=258;
		diag.Width=368;
		diag.CancelEvent = function(){
			//document.getElementById("resetflag").value = "add";
	...
	<input type="hidden" name="businessGroupId" id="businessGroupId" value="${businessGroupId }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'setProcurementOperator',1)}">
				<td><input type="button" onclick='search(0);' value="查询" class="icon_find"/></td>
				</c:if>
				<td>
				<c:if test="${my:method(login.id,'setProcurementOperator',2)}">
				<button type="button"  onclick='top.Dialog.confirm("您确定要批量设置业务员？",function(){setProcurementOperator()});' ><span class="icon_pencil">批量设置业务员</span></button>
				</c:if>
				</td>
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="searchOperatorProcurements.action" export="currentPageNumber=pageNumber" maxPageItems="50">
