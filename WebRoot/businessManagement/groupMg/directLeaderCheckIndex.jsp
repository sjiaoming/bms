...
<script type="text/javascript">
	...
	function search(offset){
		var url = "searchDirectGroups.action?pager.offset="+offset;
	...
	function openCheck(groupId){
		...
		diag.Title = "计划分组审核";
		diag.URL = "loadGroup.action?group.id="+groupId+"&flag=d";
		diag.Height=480;
		diag.Width=1720;
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'directPlanGroupMgr',1)}">
...
<thead>
	...
	<tbody>	
		...
		<tr >
			...
			<td>${group.remark }</td>
			<td>
			<c:if test="${my:method(login.id,'directPlanGroupMgr',2)}">
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="searchDirectGroups.action" export="currentPageNumber=pageNumber" maxPageItems="50">
