...
<script type="text/javascript">
	...
	function search(offset){
		var url = "searchPlans.action?pager.offset="+offset;
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			...
			<!-- 
				...
				<c:if test="${my:method(login.id,'setPlanner',1)}">
				<td><button  onclick='showProgressBar();search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
			</tr>
			<tr>
				<td colspan="9">
				<c:if test="${my:method(login.id,'setPlanner',2)}">
				<button type="button"  onclick='top.Dialog.confirm("您确定要批量设置业务员？",function(){openSetPlanner()});' ><span class="icon_pencil">批量设置业务员</span></button>
				</c:if>
				</td>
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="searchPlans.action" export="currentPageNumber=pageNumber" maxPageItems="50">
