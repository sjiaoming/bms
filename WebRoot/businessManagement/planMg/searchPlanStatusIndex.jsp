...
<script type="text/javascript">
	...
	function search(offset){
		var url = "searchPlanStatus.action?pager.offset="+offset;
	...
	*/
	function openWin2(planId){
		...
		diag.CancelEvent = function(){
			//diag.innerFrame.contentWindow.location.reload();
			window.location.href = "searchPlans.action";
			diag.close();
			};
		diag.show();
	}
	...
	function openSetPlanner(){
		...
		var diag = new top.Dialog();
		diag.Title = "批量设置";
		diag.URL = "initSetPlanner.action?tags="+tags+"&initFlag=initSetPlanner";
		diag.Height=258;
		diag.Width=368;
		diag.CancelEvent = function(){
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			...
			<!-- 
				...
				<c:if test="${my:method(login.id,'planTrack',1)}">
...
<thead>
		...
		<a herf="#" onclick="CancelAll()" >不</a>
				...
				<th width="120">采购合同号</th>
				<th width="120">销售合同号</th>
	...
	<tbody>	
		...
		<tr >
			...
			<td>
				...
				<c:if test="${plan.checkStatus==08}">内控末审核</c:if>
			...
			<td>${plan.procurementContract.contractNum }</td>
			<td>${plan.salesContract.contractNum }</td>
			<td>${plan.opinion }</td>
			<td>${plan.material }</td>
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="searchPlanStatus.action" export="currentPageNumber=pageNumber" maxPageItems="50">
