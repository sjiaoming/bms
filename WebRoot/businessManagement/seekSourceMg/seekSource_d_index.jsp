...
<script type="text/javascript">
	...
	function search(offset){
		...
		if(checkNum!="" && !reg.test(checkNum)){
			...
			var url = "seekSourceDSearch.action?pager.offset="+offset;
	...
	function batchApproval(){
			...
			seekSourceManager.checkPCStatus(tags,'09,01',function (data){
				//alert(data);
				if(data == 'no'){
					alert("请选择正确的审核状态");
					return false;
					...
					diag.URL = "initSeekSourceD.action?ssids="+tags;
					diag.Height=258;
					diag.Width=508;
					diag.CancelEvent = function(){
					document.getElementById("resetflag").value = "add";
	...
	function showElaborate(sid){
		...
		var diag = new top.Dialog();
		diag.Title = "查看详细";
		diag.URL = "showDSeekSource.action?seekSource.id="+sid;
		diag.Height=558;
		diag.Width=1590;
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'seekSourceDMg',1)}">
				<td><button  onclick='showProgressBar();search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
				<td>
				<c:if test="${my:method(login.id,'seekSourceDMg',2)}">
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="seekSourceDSearch.action" export="currentPageNumber=pageNumber" maxPageItems="50" >
