...
<script type="text/javascript">
	...
	function backSeekSource(){
			var tags = selectValidationReturnTags(true);
			if(tags==""){
				top.Dialog.alert("请选择 采购方案");
				return;
			}
				this.form1.action = "modifyProcurementsBackByIds.action?tags="+tags;
				this.form1.submit();
				return;
	}
	function openWin(){
	...
	function addSeekSource(){
		...
		var diag = new top.Dialog();
		diag.Title = "寻源";
		diag.URL = "initSeekSource.action?pc.id="+tags;
		//diag.URL = "initProcurementWay.action?pids="+tags;
		diag.Height=558;
		diag.Width=1590;
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
	...
	function openWin2(pcId){
		var diag = new top.Dialog();
		diag.Title = "审核采购方案";
		diag.URL = "doProcurementCheck.action?pcId="+pcId;
		diag.Height=480;
		diag.MessageTitle="审核采购方案";
		diag.Message="审核采购方案";
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
			var inputValue = diag.innerFrame.contentWindow.document.getElementById('cflag').value;
	...
	function exportPlans(pcId){
	     this.form1.action = "exportSeekSourcePlans.action?pcId="+pcId;
...
<div class="box2" panelTitle="功能面板 " roller="false">
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				<td>审核编号：</td>
				<td><input type="text" id="checkNum" name="pc.checkNum" value="${pc.checkNum }"/></td>
				
				<td>提交日期 从:</td>
				<td>
                  	<input  id="sDate" name="pc.beginDate"  value="<fmt:formatDate value="${pc.beginDate }" pattern="yyyy-MM-dd" />" class="date"/>
				</td>
				<td>到：</td>
				<td>
                    <input  id="eDate" name="pc.endDate"  value="<fmt:formatDate value="${pc.endDate }" pattern="yyyy-MM-dd" />" class="date"/>
				</td>
				<td>操作人：</td>
				<td><input type="text" name="personName" value="${params.personName }"/></td>
				<td>采购方案名称：</td>
				<td><input type="text" name="pc.procurementName" value="${pc.procurementName }"/></td>
				</tr>
				<tr>
				<td>方案号：</td>
				...
				<button type="button"  onclick='top.Dialog.confirm("您确定要寻源吗？",function(){backSeekSource()});' ><span class="icon_page">退回</span></button>
				</c:if>
				<c:if test="${my:method(login.id,'seekSourceMg',2)}">
				<button type="button"  onclick='top.Dialog.confirm("您确定要寻源吗？",function(){addSeekSource()});' ><span class="icon_page">寻源</span></button>
				</c:if>
				</td>
...
<thead>
	...
	<c:if test="${!empty pm.list}">
        <c:forEach items="${pm.list }" var="pch" varStatus="status">  
			...
			<td >
			<span class="img_edit hand" title="修改" onclick="exportPlans(${pch.id})"></span>
			<!-- 
			<c:if test="${my:method(login.id,'seekSourceMg',1)}">
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="procurementCheckMSearch.action" export="currentPageNumber=pageNumber" maxPageItems="50" >
