...
<script type="text/javascript">
	...
	function search(offset){
		...
		if(checkNum!="" && !reg.test(checkNum)){
			...
			var url = "seekSourceSSearch.action?pager.offset="+offset;
	...
	function showElaborate(sid){
		...
		var diag = new top.Dialog();
		diag.Title = "查看详细";
		diag.URL = "showSSeekSource.action?seekSource.id="+sid;
		diag.Height=558;
		diag.Width=870;
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'seekSourceSMg',1)}">
				<td><button  onclick='search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
				<!-- 
				<td>
				<c:if test="${my:method(login.id,'seekSourceSMg',2)}">
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="seekSourceSSearch.action" export="currentPageNumber=pageNumber" maxPageItems="50" >
