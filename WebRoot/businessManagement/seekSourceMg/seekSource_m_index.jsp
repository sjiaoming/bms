...
<script type="text/javascript">
	...
	function search(offset){
		...
		if(checkNum!="" && !reg.test(checkNum)){
			...
			var url = "seekSourceMSearch.action?pager.offset="+offset;
	...
	function batchApproval(){
			...
			seekSourceManager.checkPCStatus(tags,'06,03',function (data){
				//alert(data);
				if(data == 'no'){
					alert("请选择正确的审核状态");
					return false;
					...
					diag.Title = "批量审批寻源";
					//diag.URL = "businessManagement/seekSourceMg/seekSource_m_check.jsp?ssids="+tags;
					diag.URL = "initSeekSourceM.action?ssids="+tags;
					diag.Height=258;
					diag.Width=508;
	...
	function showElaborate(sid){
		...
		var diag = new top.Dialog();
		diag.Title = "查看详细";
		diag.URL = "showMSeekSource.action?seekSource.id="+sid;
		diag.Height=558;
		diag.Width=1590;
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'seekSourceMMg',1)}">
				<td><button  onclick='showProgressBar();search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
				<td colspan="8">
				<c:if test="${my:method(login.id,'seekSourceMMg',2)}">
				<button type="button"  onclick='top.Dialog.confirm("您确定要重新选择？",function(){batchApproval()});' ><span class="icon_page">批量审批</span></button>
				</c:if>
				</td>
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="seekSourceMSearch.action" export="currentPageNumber=pageNumber" maxPageItems="50" >
