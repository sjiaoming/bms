...
<script type="text/javascript">
	...
	function search(offset){
		...
		if(checkNum!="" && !reg.test(checkNum)){
			...
			var url = "seekSourceLSearch.action?pager.offset="+offset;
	...
	function batchApproval(){
			...
			seekSourceManager.checkPCStatus(tags,'07',function (data){
				//alert(data);
				if(data == 'no'){
					alert("请选择正确的审核状态");
					return false;
					...
					var diag = new top.Dialog();
					diag.Title = "批量寻源审核";
					diag.URL = "businessManagement/seekSourceMg/seekSource_l_check.jsp?ssids="+tags;
					//diag.URL = "initSeekSourceL.action?ssids="+tags;
					diag.Height=258;
					diag.Width=508;
					diag.CancelEvent = function(){
						document.getElementById("resetflag").value = "add";
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'seekSourceLMg',1)}">
				<td><button  onclick='search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
				<td>
				<c:if test="${my:method(login.id,'seekSourceLMg',2)}">
