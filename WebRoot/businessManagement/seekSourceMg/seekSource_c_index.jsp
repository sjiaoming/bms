...
<script type="text/javascript">
	...
	function search(offset){
		...
		if(checkNum!="" && !reg.test(checkNum)){
			...
			var url = "seekSourceCSearch.action?pager.offset="+offset;
	...
	function batchApproval(){
			...
			diag.Title = "批量审批寻源";
			diag.URL = "businessManagement/seekSourceMg/seekSource_c_check.jsp?ssids="+tags;
	...
	function showElaborate(sid){
		...
		var diag = new top.Dialog();
		diag.Title = "查看详细";
		diag.URL = "showCSeekSource.action?seekSource.id="+sid;
		diag.Height=558;
		diag.Width=1590;
		diag.CancelEvent = function(){
			document.getElementById("resetflag").value = "add";
	...
	<input type="hidden" name="resetflag" id="resetflag" value="${resetflag }" />
		<table>
			<tr>
				...
				<c:if test="${my:method(login.id,'seekSourceCMg',1)}">
				<td><button  onclick='showProgressBar();search(0);'><span class="icon_find">查询</span></button></td>
				</c:if>
				<td colspan="8">
				<c:if test="${my:method(login.id,'seekSourceCMg',2)}">
...
<div style="height:35px;">
	<pg:pager items="${pm.total }" url="seekSourceCSearch.action" export="currentPageNumber=pageNumber" maxPageItems="50" >
